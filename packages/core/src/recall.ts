...
export class RecallEngine {
  ...
  async recall(
    ...
    // ─── Ablation harness (benchmark IP — LongMemEval / LoCoMo) ────────
    // CELIUMS_RECALL_DISABLE_AFFECT=1 removes the affective channel
    // (emotionalWeight + limbicResonance + the SAR arousal redistribution)
    // and RENORMALISES the remaining weights to sum to 1, so the ablation
    // isolates "does the affective signal help retrieval?" instead of
    // merely shrinking every score (which would silently change what the
    // scoreThreshold means). Default OFF → byte-identical to current
    // behaviour, safe to ship to prod unflagged.
    const affectOff = process.env['CELIUMS_RECALL_DISABLE_AFFECT'] === '1';
    const W = this.config.weights;
    const ablBase = W.semantic + W.textMatch + W.importance + W.retrievability;
    const ablW = affectOff && ablBase > 0
      ? {
          semantic: W.semantic / ablBase,
          textMatch: W.textMatch / ablBase,
          importance: W.importance / ablBase,
          retrievability: W.retrievability / ablBase,
        }
      : null;
    ...
    const scored: ScoredMemory[] = memories.map((memory: any) => {
      ...
      const finalScore = ablW
        ? // AFFECT ABLATED: renormalised non-affective channels only, no
          // SAR redistribution, zero emotional/resonance contribution.
          ablW.semantic * semanticScore +
          ablW.textMatch * textMatchScore +
          ablW.importance * importanceScore +
          ablW.retrievability * retrievabilityScore
