...
export function computeCircadianFor(
  ...
  // Ablation harness (LongMemEval / LoCoMo). CELIUMS_RECALL_DISABLE_CIRCADIAN=1
  // zeroes ONLY the time-of-day rhythm term, keeping baseArousal + the
  // factor channel (affect/session/etc.) intact — so the ablation isolates
  // "does the circadian rhythm modulation help?" independently of the
  // affective ablation. Default OFF → identical to current behaviour.
  const circadianOff = process.env['CELIUMS_RECALL_DISABLE_CIRCADIAN'] === '1';
  const circadianContribution = circadianOff
    ? 0
    : config.amplitude * rhythmComponent * lethargyFactor;
