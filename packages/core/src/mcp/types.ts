...
export interface McpToolContext {
  ...
  /** Composite Track 1 runtime — storage + sync + AAL + audit (see
   *  lib/runtime/context.ts). Handlers opt in by reading `ctx.runtime`;
   *  legacy handlers ignore it. */
  runtime?: import('../lib/runtime/index.js').RuntimeContext;
  /** Principal — set by the auth layer (ADR-003) once per request. */
  principal?: import('../lib/auth/types.js').Principal;
  /** Caller-supplied AAL confirm token, propagated from HTTP header
   *  X-Celiums-AAL-Confirm. */
  aalConfirmToken?: string;
  /** Caller-supplied AAL override reason, propagated from HTTP header
   *  X-Celiums-AAL-Override. Only honoured for platform-owner role. */
  aalOverrideReason?: string;
  /** Caller-supplied approved-pending id, propagated from HTTP header
   *  X-Celiums-AAL-Pending-Id. */
