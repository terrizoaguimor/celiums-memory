...
const handleProjectList: McpToolHandler = async (args, ctx) => {
  ...
  const userId = String(args.userId ?? ctx.userId);
  const r = await pool.query(
    `SELECT p.id, p.title, p.genre, p.structure_template, p.status,
            p.word_target, p.created_at, p.updated_at,
            (SELECT COUNT(*) FROM write_scenes s WHERE s.project_id = p.id)     AS scene_count,
            (SELECT COUNT(*) FROM write_characters c WHERE c.project_id = p.id) AS character_count,
            (SELECT COALESCE(SUM(word_count), 0) FROM write_scenes s WHERE s.project_id = p.id) AS word_count
       FROM write_projects p
      WHERE p.user_id = $1
      ORDER BY p.updated_at DESC`,
    [userId],
  );
  return ok(asText({ projects: r.rows }));
};
...
export const WRITE_TOOLS: RegisteredTool[] = [
  ...
  },
  {
    group: 'ai',
    definition: {
      name: 'write_project_list',
      description: 'List all writing projects for a user, with scene count, character count, and total word count. Use to discover what manuscripts are in progress before opening one with write_project_get.',
